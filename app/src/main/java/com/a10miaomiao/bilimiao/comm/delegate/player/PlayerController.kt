...
class PlayerController(
    ...
    private var lastPauseTime = 0L // 记录上次暂停时间，用于检测后台超时
    ...
    override fun onAutoCompletion() {
        ...
        scope.launch {
            ...
            if (playListState.items.isNotEmpty()) {
                ...
                if (currentIndex >= 0 && currentIndex + 1 < playListState.items.size) {
                    val nextItem = playListState.items[currentIndex + 1]
                    delegate.openPlayer(nextItem.toVideoPlayerSource())
                    return@launch
                }
            }
            ...
            if (isLoop) {
                ...
                PlaybackService.instance?.unbindPlayer()
                PlaybackService.instance?.clearMetadata()
            }
        }
    }
    ...
    override fun onVideoResume(isResume: Boolean) {
        if (isResume) {
            checkResumeAfterBackground()
        }
    }

    /**
     * 由 Activity 生命周期（onResume）调用，检测播放器是否需要重建
     * 同时检测：超时30分钟 OR 播放器处于错误状态
     */
    fun checkResumeAfterBackground() {
        val now = System.currentTimeMillis()
        val player = views.videoPlayer
        val isError = player?.currentState == GSYVideoView.CURRENT_STATE_ERROR
        val isTimeout = lastPauseTime > 0 && (now - lastPauseTime) > 30 * 60 * 1000L

        if (isError || isTimeout) {
            lastPauseTime = 0L
            val source = delegate.playerSource
            if (source != null && delegate.isOpened()) {
                delegate.closePlayer()
                delegate.openPlayer(source)
            }
        }
    }
    ...
    override fun setStateAndUi(state: Int) {
        ...
        // 同步通知栏播放状态（作为 onVideoPause/onVideoResume 的兜底保障）
        when (state) {
            GSYVideoView.CURRENT_STATE_PLAYING -> PlaybackService.instance?.setPlaying(true)
            GSYVideoView.CURRENT_STATE_PAUSE -> PlaybackService.instance?.setPlaying(false)
            GSYVideoView.CURRENT_STATE_AUTO_COMPLETE -> PlaybackService.instance?.setPlaying(false)
        }
    }
