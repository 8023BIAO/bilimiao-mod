...
class PlayerDelegate2(
    ...
    private val player: DanmakuVideoPlayer get() = views.videoPlayer ?: error("video_player not found in layout")
    ...
    private fun startPlaybackService() {
        try {
            ...
            // startForegroundService 异步，必须 post 等 Service.onCreate 执行完再设 delegate
            Handler(Looper.getMainLooper()).post {
                PlaybackService.instance?.setPlayerDelegate(this@PlayerDelegate2)
            }
        } catch (e: Exception) {
            miaoLogger().e("startPlaybackService failed: ${e.message}")
        }
    }

    override fun onResume() {
        // 后台检测：恢复时若播放器出错则重建避免黑屏
        controller.checkResumeAfterBackground()
    }

    override fun onPause() {
        // 最早时机尝试进小窗 — onUserLeaveHint 在国产ROM上时机不可靠（Home vs 切APP不同）
    ...
    override fun onStart() {
        if (!controller.isBackgroundPlay) {
            ...
            if (p.isInPlayingState) {
                if (isPlaying()) {
                    // 播放中回到前台：正常恢复
                    p.onVideoResume()
                } else {
                    // 暂停中回到前台：Surface 可能已销毁，先 resume 重连再恢复暂停
                    p.onVideoResume()
                    p.onVideoPause()
                }
            }
    ...
    override fun onStop() {
        // 始终尝试 PiP — onPause 在部分 ROM 上可能失败（如最近任务切换）
        if (controller.isPipOnBackground && isOpened()) {
            tryEnterPipOnBackground()
        } else if (!controller.isBackgroundPlay
            && !controller.isPipOnBackground) {
            val p = views.videoPlayer ?: return
            if (p.isInPlayingState) {
                p.onVideoPause()
            }
...
//        }
    ...
    suspend fun loadPlayerSource(
        ...
        try {
            ...
            // 自动联播/切清晰度/reload 后刷新通知栏信息
            PlaybackService.instance?.let { svc ->
                svc.updateMetadata(source.title, source.ownerName, source.coverUrl)
                if (showNotification) {
                    svc.setPlaying(true)
                }
            }
        ...
        scaffoldApp.post {
            ...
            playerCoroutineScope.launch(Dispatchers.Main) {
                SettingPreferences.getData(activity) {
                    ...
                    showNotification = it[PlayerNotification] ?: false
                }
                if (showNotification) {
