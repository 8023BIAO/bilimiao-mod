...
class AppBarBehavior : CoordinatorLayout.Behavior<View> {
    ...
    companion object {
        /**
         * 全局锁状态 — 跨 Behavior 实例持久化。
         * Behavior 可能在 ScaffoldView 重建/重布局时被重新创建，
         * 此时实例的 bottomBarLocked 会回到默认 false。
         */
        @Volatile
    ...
    /**
     * 锁定底栏，不随滚动/页面切换隐藏。
     * 额外存一份在 Behavior 上，防止 parentRef null 时（视图重建等）检查失效。
     */
    var bottomBarLocked = false

    override fun onLayoutChild(parent: CoordinatorLayout, child: View, layoutDirection: Int): Boolean {
        this.viewRef = child
        // 每次布局重建时同步全局锁到实例 + 强制可见 — 防止任何机制在锁定时隐藏底栏
        if (globalLock) {
        ...
        if (parent is ScaffoldView) {
            ...
            if (parent.fullScreenPlayer) {
                ...
                // 底栏锁定时，强制重置状态和位置为可见
                if (bottomBarLocked || globalLock || parent.bottomBarLocked) {
                    currentState = STATE_SCROLLED_UP
                    child.translationY = 0f
                    // 跳过动画直接显示菜单按钮，避免按钮不出现
                    if (child is AppBarView) {
                        child.showMenu()
                    }
                } else if (currentState == STATE_SCROLLED_DOWN
            ...
            // 非 ScaffoldView 父容器下也保护底栏锁定
            if (globalLock || bottomBarLocked) {
                currentState = STATE_SCROLLED_UP
                child.translationY = 0f
            }
        }
    ...
    override fun onStartNestedScroll(
        ...
        return viewRef?.top != 0 // AppBar在底部时
                ...
                && !(parentRef?.bottomBarLocked ?: false) // 未锁定底栏（取自ScaffoldView）
                && !bottomBarLocked // 未锁定底栏（自身备份）
                && !globalLock // 未锁定底栏（全局）
    ...
    fun slideDown(child: View, animate: Boolean) {
        // 锁定底栏时禁止任何方式隐藏（三层保护）
        if (bottomBarLocked || parentRef?.bottomBarLocked == true || globalLock) {
            return
        }
        if (isScrolledDown()) {
            return
        ...
        val targetTranslationY = appBarHeight + child.paddingBottom + additionalHiddenOffsetY
        if (animate) {
            animateChildTo(
                child,
                targetTranslationY,
